// Test the final keyword extraction logic
...
console.log('🔍 Testing final keyword extraction logic:\n');
...
async function testFinalKeywordSearch() {
  console.log('\n🔍 Testing final keyword-based search:\n');
  ...
  if (keywordError) {
    ...
    if (keywordData?.length > 0) {
      ...
      if (dogAnswers.length > 0) {
        ...
        console.log('The memory service should now work correctly for Jonathan demo.');
      } else {
        console.log('\n⚠️  Still not finding dog answers with combined keywords.');
        console.log('Let me test individual keywords...');
        ...
        for (const keyword of keywords) {
          ...
          if (!singleError && singleData) {
            ...
            if (singleDogAnswers.length > 0) {
              ...
              singleDogAnswers.forEach((answer, j) => {
                console.log(`    ${j+1}. ${answer.fragment_text.substring(0, 80)}...`);
              });
            }
          }
        }
...
testFinalKeywordSearch().catch(console.error);