...
export async function POST(req: NextRequest) {
  try {
    ...
    elevenForm.append(
      'description',
      `American English voice clone for ${name} using ${audioFiles.length} audio samples. Trained for US English pronunciation. Script: ${script?.slice(0, 80) || 'No script provided'}`
    )
    
    // Force American English with multiple approaches
